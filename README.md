...
这种情况下，每一个打印需求都需要你的用户手动触发。那么，可以让这个流程自动化吗？即我们预先配置好打印机、打印格式等，当打印需求产生时不需要用户手动触发打印，而是让打印自动完成。仅仅依靠网页是无法完成的，这个小工具的作用就是帮助你实现网页的自动化打印，它是一个客户端软件，你需要把它安装在能连接相应打印机的电脑上。我们来看看它的组成。

它使用C++开发，使用了qt 和drogon。

你最可能的做法是做一个页面，,当你的用户做出类似于点击一个按钮这样的操作时，，打开浏览器的打印预览对话框，之后。
